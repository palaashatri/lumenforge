...
public class SDTurboPipeline {
    ...
    public SDTurboPipeline(OrtEnvironment env, OrtSession unetSession, OrtSession vaeSession) {
        this.env = env;
        this.unet = new OnnxUNet(env, unetSession);
        this.vae = new OnnxVAE(env, vaeSession, null);
        this.scheduler = new EulerScheduler();
        this.sampler = new SchedulerSampler();
    }

    public FloatTensor run(InferenceRequest request, FloatTensor textEmbeddings) {
        int width = Math.max(256, (request.width() / 8) * 8);
        int height = Math.max(256, (request.height() / 8) * 8);
        int latentW = width / 8;
        int latentH = height / 8;
        int steps = Math.max(1, request.batch() > 0 ? request.batch() : 4);
        ...
        SamplerContext ctx = new SamplerContext(
                ...
                textEmbeddings,
                steps,
                1.0f, // No CFG for Turbo
                request.progressCallback() != null ? (step, progress, elapsed) -> {
                    request.reportProgress("Denoising: " + step + "/" + steps + " steps (Modular Turbo)");
                } : null,
                new HashMap<>()
        );
